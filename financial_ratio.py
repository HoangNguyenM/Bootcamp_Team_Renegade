...
def combine_data(df, data):
    ...
    for j in range(5):
        for i in range(number_of_points):
            combined_data.loc[len(rng)+j,'Feature'] = 'day '+str(j-1)
...
combined_data.to_csv("D:/Dropbox/Code/Bootcamp/financial_data.csv",index=False)

#%%
print(len(combined_data.axes[0]))
print(len(combined_data.axes[1]))

#%%
import matplotlib.pyplot as plt

mat = all_data[0][all_data[0].columns[1:]].to_numpy()
